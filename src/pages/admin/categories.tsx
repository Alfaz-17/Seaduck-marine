...
export default function AdminCategoryPage() {
  ...
  const handleAddCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewCategoryName('');
      setMessage({ type: 'success', text: 'New category registered.' });
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Delete this category? This may affect linked assets.')) return;
    try {
  ...
  if (isLoading) return <div className="text-xs font-mono font-bold uppercase tracking-widest animate-pulse text-primary-light">Scanning Registries...</div>;

  return (
    <div className="max-w-4xl space-y-12 font-sans">
      {/* Header */}
      <div className="border-b border-slate-800 pb-8 relative">
        <h1 className="text-3xl font-extrabold text-white uppercase tracking-tighter font-syne m-0">Category Management</h1>
      ...
      <div className="bg-primary/55 p-10 border border-primary-light/20 relative">
         ...
         <h2 className="text-sm font-mono font-bold uppercase tracking-widest text-primary-light mb-6 m-0">Register New Category</h2>
         <form onSubmit={handleAddCategory} className="flex flex-col sm:flex-row gap-4">
            <input 
              type="text" 
              placeholder="Category Name (e.g. Navigation Systems)" 
      ...
      <div className="bg-primary/20 border border-slate-800 shadow-2xl">
         <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-800 font-mono text-xs text-white">
               ...
               {categories.length === 0 && (
                  <tr>
                     <td colSpan={2} className="py-20 text-center text-xs font-bold text-slate-500 uppercase tracking-widest italic">
                       No categories registered in the grid
                     </td>
                  </tr>
               )}
            </tbody>
